      ...
      context("querying for non-existent key", () => {
        it("throws a ReferenceError", () => {
          ...
          expect(() => {
            ...
            ReferenceError,
            `The key 'fruit' has not been transposed`
          );
        });
      });
    });
...
class Fixture {
  ...
  public thenTransposedEquals<T, K extends keyof T>(
    transposed: Transposed<T, K>,
    type?: string
  ) {
    expect(this._oatyArray!.transposed).to.deep.equal(transposed);
    if (type === "Dummy") {
      expect(
        (this._oatyArray!.transposed! as unknown as any).data!.hello![0]
      ).to.be.an.instanceOf(Dummy);
    }
  }
